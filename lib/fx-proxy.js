...
async function fetchFxChart(interval = "1d") {
  ...
  if (meta) {
    ...
    return { ...chart, interval, intervalLabel: meta.label, pair: "USD/KRW" };
  }

  if (interval === "1M") {
    const chart = await fetchFrankfurterMonthly(24);
    return { ...chart, interval: "1M", intervalLabel: "1월", pair: "USD/KRW" };
  }

  const chart = await fetchFrankfurterDaily(90);
  return { ...chart, interval: "1d", intervalLabel: "1일", pair: "USD/KRW" };
}
