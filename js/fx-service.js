...
function fxServerHint() {
  if (window.location.protocol === "file:") {
    return "file:// 로는 안 됩니다 → python server.py";
  }
  if (/vercel\.app$/i.test(window.location.hostname)) {
    return "Vercel 재배포 또는 /api/fx/rates 확인";
  }
  return "python server.py 실행 여부 확인";
}
...
function renderFxEmpty(detail) {
  ...
  const list = document.getElementById("fx-rates");
  if (list) {
    const msg = detail ? `${detail} — ${fxServerHint()}` : `환율 API 연결 실패 — ${fxServerHint()}`;
    list.innerHTML = `<div class="news-empty">${msg}</div>`;
  }
